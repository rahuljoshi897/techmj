...
/**
 * This class is the "Breadcrumb Trail".
 * We wrapped it into the "BF_Breadcrumb_Parent" to make sure the will not be any another conflict with
 * other versions of it.
 *
 * Changes:
 * 1- The 'breadcrumb-trail' textdomain changed to 'better-studio' to only make it easy to translate for users
 *    (only 1 textdomain to translate)
 *
 * 2- 'breadcrumb_trail' prefix for filters changed with 'bf_breadcrumb'
 *
 * Home:
 * https://github.com/aliaghdam/breadcrumb-trail
 * -------------------------------------------------------------
 *
 ...
 */
abstract class BF_Breadcrumb_Parent {
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'bf_breadcrumb_args', wp_parse_args( $args, $defaults ) );
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			foreach ( $this->items as $item ) {
				...
				$item = ! empty( $matches ) ? sprintf( '%s<span itemprop="name">%s</span>%s', $matches[1], $matches[2], $matches[3] ) : sprintf( '<p itemscope itemtype="http://schema.org/Thing" itemprop="item"><span itemprop="name">%s</span></p>', $item );
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( TRUE === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'publisher' ) ) );
		}
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( TRUE === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'publisher' ) ) );
		}
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( TRUE === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'publisher' ) ) );
		}
	}
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag ) {
					$this->items[] = sprintf( $this->a_format, esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'publisher' ) ) ) );
				} // If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag ) {
					$this->items[] = sprintf( $this->a_format, esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'publisher' ) ) ) );
				} // If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag ) {
					$this->items[] = sprintf( $this->a_format, esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'publisher' ) ) ) );
...
/**
 * BF Customization on "Breadcrumb Trail".
 */
class BF_Breadcrumb extends BF_Breadcrumb_Parent {

	/**
	 * BF_Breadcrumb constructor.
	 */
	 ...
	 * 2- 'breadcrumb_trail' filter removed
	 * 3- 'clearfix' added to main wrapper
	 * 4- 'custom_class' support fo main wrapper
	 * ----
	 *
	 * Formats the HTML output for the breadcrumb trail.
	 *
	...
	/**
	 * changed to show BS main category
	 *
	 * @since  1.0.0
	 * @access protected
	 *
