...
if ( ! function_exists( 'bf_is_doing_ajax' ) ) {
	...
	function bf_is_doing_ajax( $ajax_action = '' ) {
		...
		if ( empty( $ajax_action ) ) {
			...
			return ! empty( $_REQUEST['action'] ) && $_REQUEST['action'] === $ajax_action;
		}
	}
}
...
if ( ! function_exists( 'bf_remove_notice' ) ) {
	 ...
	 * @return bool true on success or false on error.
	 */
	function bf_remove_notice( $notice ) {
		return Better_Framework()->admin_notices()->remove_notice( $notice );
	}
}
		...
		switch ( $site_id ) {
			...
			case 'facebook':
				...
				if ( ! empty( $fb_api['id'] ) && ! empty( $fb_api['secret'] ) ) {
					$api_url = 'https://graph.facebook.com/v2.7/?id=' . urlencode( $url ) . '&access_token=' . $fb_api['id'] . '|' . $fb_api['secret'];
				} else {
