...
class Better_Framework {
	...
	public $sections = array(
		...
		'minify'                 => TRUE,    // For initializing static files compressor
	...
	public static function factory( $object = 'options', $fresh = FALSE, $just_include = FALSE ) {
		...
		switch ( $object ) {
			...
			case 'minify':
				...
				include BF_PATH . 'minify/class-bf-minify.php';
				include BF_PATH . 'minify/class-bf-scripts.php';
				include BF_PATH . 'minify/class-bf-styles.php';

				include BF_PATH . 'minify/minify-functions.php';
		...
		try {
			...
			switch ( $_REQUEST['reqID'] ) {
				...
				case 'fetch-deferred-field':
					...
					} else if ( ! empty( $_REQUEST['sectionID'] ) &&
